const POSTGREST_URL = 'http://localhost:8000/pgrst';
...
async function run() {
    console.log("Fetching mentions from database...");
    let allMentions = [];
    let offset = 0;
    const limit = 1000;
    while (true) {
        const res = await fetch(`${POSTGREST_URL}/mentions?limit=${limit}&offset=${offset}&order=mention_id.asc`, { headers: API_HEADERS });
        if (!res.ok) throw new Error('Failed to fetch mentions: ' + res.status + ' ' + await res.text());
        const data = await res.json();
        if (data.length === 0) break;
        allMentions = allMentions.concat(data);
        console.log(`Loaded ${allMentions.length} mentions...`);
    ...
    console.log(`Found ${updates.length} / ${allMentions.length} mentions requiring normalization updates.`);
    ...
    for (let i = 0; i < updates.length; i += CHUNK_SIZE) {
        ...
        if (!res.ok) throw new Error('Failed to save bulk: ' + res.status + ' ' + await res.text());
        processed += chunk.length;
        console.log(`Updated ${processed} / ${updates.length} mentions.`);
    }
