...
**FORMAT and SCHEMA MARKDOWN FILES**
	...
	When a source is selected, preview the first 30 rows of the source.
	Ad a check box that says "ingest only 50 rows" after source file is loaded.
	default it checked.
	If it is checked  process only the first 50 rows of the source file.
	Adda a pulldoen menu to select the county. The following are the counties to choose from {
		display_name : "Albemarle" value : "ALB"
		display_name : "Augusta" value : "AUG"
		display_name : "Fauquier" value : "FAU"
		}

**INGEST PROCESS **

	A file containing the sources available is called "sources.csv". 
	Each one will need its own format markdown and translation function, which the ingest process will be routed through. 
	Each row has the following fields {
		...
		county is the county of the source file.
		state is the state of the source file.
		}

When a format markdown file (such as 1870CensusFormat.md) is used to generate the translation function, that translation function is appended to the end of "Format.md". 
...
The process for ingesting a source is:
	...
	When the user selects the row {
		...
		For each row in loaded table {
			...
			Add county code from source.csv into county.
